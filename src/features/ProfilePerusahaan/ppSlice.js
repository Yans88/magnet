...
export const simpanDataPP = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        ...
        try {
            const response = await axios.post(API_URL + '/action-data-profile-perusahaan', param, config);
            let data = '';
...
export const ppSlice = createSlice({
    ...
    extraReducers: {
        ...
        [getDataPP.fulfilled]: (state, { payload }) => {
            ...
            state.persetujuan = payload.persetujuan ? payload.persetujuan : { agree1: '', agree2: '', data_profile_perusahaan_id: '' };
		...
		[action_contact_us.fulfilled]: (state, { payload }) => {
            ...
            state.contentMsg = "<div style='font-size:20px; text-align:center; line-height:23px;'><strong>Terima kasih</strong><br/> Pesan anda telah terkirim, tim kami akan segera menjawab pesan anda</div>";
